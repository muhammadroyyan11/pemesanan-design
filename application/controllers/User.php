...
class User extends CI_Controller
{
	...
	{
		// $id = $this->uri->segment(3);
		$main['main'] = 'user/menu/chat';
		$main['header'] = 'Desain Grafis';
		$main['data'] = $this->muser->get_chat($_SESSION['pembeli']['id'], 1)->result_array();
		$this->load->view('user/index', $main);
		// echo "sini user";
		// var_dump($main['data']);
	}
	...
	function beli()
	{
		...
		if ($this->uri->segment(3) == 'beli_produk') {
			if ($this->input->post('id_nya') != '' and $this->input->post('data') != '' and $this->input->post('id_nya') != null and $this->input->post('data') != null) {
				...
				$this->muser->insert('tb_pembelian', array('id_pembeli' => $data_pembeli['id'], 'id_produk' => $id, 'id_transaksi' => 1, 'deadline' => $deadline, 'pembayaran' => $pembayaran, 'ket' => json_encode($data)));
	...
	function pesanan()
	{
		...
		if ($this->input->post('tambah_ket')) {
			...
			if ($this->input->post('id_nya') != '' and $this->input->post('data') != '' and $this->input->post('id_nya') != null and $this->input->post('data') != null) {
				...
				$this->muser->update('tb_pembelian', array('no' => $id), array('deadline' => $deadline, 'pembayaran' => $pembayaran,  'ket' => json_encode($data)));
				// echo $value->no;
				// print_r(base_url().'user/pesanan/detail/'.$value->no);
				$url = base_url() . 'user/pesanan/detail/' . $id;
				$array = array('url' => $url, 'error' => 3);
		...
		} elseif ($this->uri->segment(3) == 'upload_foto_pelunasan') {
			$file = $_FILES['file'];
			if ($file == '' or $file == null) {
				redirect('/user');
			} else {
				...
				///// cek folder jika ada @ tiada ///////
				$no = $this->uri->segment(4);
				$dir = 'images/pembelian/foto_pelunasan/';
				if (is_dir($dir) === false) {
					mkdir($dir);
				}

				print_r($no);
				$cek_data = $this->muser->tampil_data_where('tb_pembelian', array('no' => $no));
				$this->muser->update('tb_pembelian', array('no' => $no), array('bukti_pelunasan' => $dir . $no . '.jpg'));

				$this->muser->update('tb_pembelian', array('no' => $no), array('id_transaksi' => 13));
				$this->session->set_flashdata('success', '<b>Foto Transaksi Pelunasan Berhasil Diupload</b><br>Pengesahan Transaksi Pembayaran Sedang Dalam Proses<br> Mohon Bersabar , Terima Kasih');
	...
	function desain_sendiri()
	{
		...
		if (is_numeric($this->uri->segment(3))) {
			...
			if ($this->input->post('data') != '' and $this->input->post('data') != null) {
				...
				// $this->muser->insert('tb_pembelian',array('id_pembeli' => $data_pembeli['id'], 'id_produk' => $this->uri->segment(3), 'id_transaksi' => 1, 'deadline' => $deadline, 'ket' => json_encode($data)));
				$this->muser->insert('tb_pembelian', array('desain' => 1, 'kategori' => $kategorinya, 'id_pembeli' => $data_pembeli['id'], 'id_transaksi' => 1, 'deadline' => $deadline, 'pembayaran' => $pembayaran, 'ket' => json_encode($data)));
				$cek_data = $this->muser->tampil_data_last1('tb_pembelian', 'no', array('id_pembeli' => $data_pembeli['id']));
