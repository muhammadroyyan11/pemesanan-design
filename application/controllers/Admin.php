...
class Admin extends CI_Controller
{
	...
	public function chat_index()
	{
		...
		$main['main'] = 'admin/menu/chat_index';
		$main['header'] = 'Halaman Utama';
	...
	function proses_pembuatan()
	{
		if ($this->uri->segment(3) == 'tanggalnya') {
			...
			if ($file == '' or $file == null) {
				...
				$jenis_pembayaran = $this->madmin->tampil_data_where('tb_pembelian', array('no' => $no))->result_array();

				if ($jenis_pembayaran[0]['pembayaran'] != 'DP') {
					...
					$this->madmin->update('tb_pembelian', array('no' => $no), array('id_transaksi' => 12));
					$this->session->set_flashdata('success', '<b>Foto Desain Berhasil Diupload</b><br>Menunggu Pelunasan Dari Pembeli<br> Mohon Bersabar , Terima Kasih');
				}

				// print_r($jenis_pembayaran[0]['pembayaran']);

				move_uploaded_file($file["tmp_name"], $dir . $no . ".jpg");
			}
		} elseif ($this->uri->segment(3) == 'konfirmasi') {
			$no = $this->uri->segment(4);
			$cek_data = $this->madmin->tampil_data_where('tb_pembelian', array('no' => $no, 'id_transaksi' => 13));

			if (count($cek_data->result()) > 0) {
				$this->madmin->update('tb_pembelian', array('no' => $no), array('id_transaksi' => 4));
				$this->session->set_flashdata('success', "<b>Berhasil Disahkan</b><br>Konfirmasi Pembayaran Telah Berhasil Disahkan<br>Pembelian No " . $no . " Mununggu Konfirmasi dari Pembeli<br>");
				redirect('/admin/daftar_tunggu');
			} else {
				redirect('/admin/daftar_tunggu');
			}
	...
	function daftar_pembelian()
	{
		if (is_numeric($this->uri->segment(3))) {
			...
			if (count($cek_data->result()) > 0) {
				$main['header'] = 'Halaman Daftar Pembelian';
				$main['data_pembelian'] = $cek_data;
				foreach ($cek_data->result() as $key => $value);
				$main['data_pembeli'] = $this->madmin->tampil_data_where('tb_pembeli', array('id' => $value->id_pembeli));
				$main['bukti_pembayaran'] = $this->madmin->tampil_data_where('tb_foto_pembelian', array('no' => $value->no));
				...
				foreach ($cek_data->result() as $key => $value) {
					...
					if ($value->desain == 0) {
						if ($value1->kategori != 3) {
							$jumlah_harga = ($ket_produk->harga  * $ket_pembelian->jumlah_kertas) + $ket_produk->harga  + substr($value2->no_telpon, -3);
							if ($value1->kategori == 1) {
